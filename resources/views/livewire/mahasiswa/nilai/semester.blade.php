@section('title', 'Nilai Semester — ' . config('app.name'))
@section('header_title', 'Nilai Semester')
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div class="rounded-2xl bg-white p-6 shadow-border">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-xs text-neutral-500">IP Kumulatif</div>
                    <div class="text-2xl font-bold text-amber-600">{{ $formatIp($data['statistik']['ip_kumulatif']) }}</div>
                </div>
            </div>
        </div>
    </div>

    @if (count($data['transkrip']) === 0)
        <div class="rounded-2xl bg-white p-10 text-center shadow-border">
            ...
            <p class="mt-1 text-sm text-neutral-500">Belum ada mata kuliah yang sudah disetujui dan memiliki nilai.</p>
        </div>
    @else
        <div class="space-y-6">
            @foreach ($data['transkrip'] as $group)
                <div wire:key="semester-{{ $group['semester']->id }}" class="overflow-hidden rounded-2xl bg-white shadow-border">
                    <div class="flex items-center justify-between border-b border-neutral-100 bg-sky-50 px-6 py-4">
                        <div>
                            <h3 class="text-lg font-semibold text-neutral-900">{{ $group['semester']->nama }}</h3>
                            <p class="text-sm text-neutral-500">{{ $group['semester']->kode }}</p>
                        </div>
                        <div class="text-right">
                            <div class="mb-1 text-xs text-neutral-500">IP Semester</div>
                            <div class="text-2xl font-bold text-sky-600">{{ $formatIp($group['ip_semester']) }}</div>
                            <div class="mt-1 text-xs text-neutral-500">{{ $group['total_sks'] }} SKS</div>
                        </div>
                    </div>
                                ...
                                @foreach ($group['mata_kuliah'] as $mk)
                                    <tr wire:key="mk-{{ $mk['id_krs'] }}" class="hover:bg-neutral-50">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            ...
                                            @else
                                                <span class="text-xs text-neutral-400">Belum ada nilai</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
