...
@section('header_title', 'Permission')
@section('header_subtitle', 'Daftar permission granular panel admin')
...
@section('page_actions')
    <a
        href="{{ route('admin.pengguna.permission.create') }}"
                ...
                <tbody class="divide-y divide-neutral-100">
                    @forelse ($permissionList as $permission)
                        <tr wire:key="permission-{{ $permission->id }}">
                            <td class="px-4 py-3 font-medium text-neutral-900">{{ $permission->name }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <a
                                        href="{{ route('admin.pengguna.permission.edit', $permission->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-10 text-center text-neutral-500">Belum ada data permission.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/40 px-4">
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-border-lg">
                <h3 class="text-base font-semibold text-neutral-900">Hapus permission?</h3>
