...
@section('header_subtitle', 'Daftar role akses panel admin')
@section('header_icon', 'shield')
...
@section('page_actions')
    <a
        href="{{ route('admin.pengguna.role.create') }}"
                ...
                <tbody class="divide-y divide-neutral-100">
                    @forelse ($roleList as $role)
                        <tr wire:key="role-{{ $role->id }}">
                            <td class="px-4 py-3 font-medium text-neutral-900">{{ $role->code }}</td>
                            <td class="px-4 py-3 text-neutral-600">{{ $role->name }}</td>
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <a
                                        href="{{ route('admin.pengguna.role.edit', $role->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-10 text-center text-neutral-500">Belum ada data role.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/40 px-4">
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-border-lg">
                <h3 class="text-base font-semibold text-neutral-900">Hapus role?</h3>
