...
<div>
    <div class="mb-4 flex flex-wrap items-center justify-end gap-2">
        ...
        <button
            type="button"
            wire:click="confirmDeleteKurikulum"
    ...
    <div class="mt-6 rounded-2xl bg-white p-6 shadow-border">
        ...
        @if ($kurikulum->matkuls->count() > 0 && $this->matkulTanpaBobotCount > 0)
            <div class="mb-4 flex flex-col gap-3 rounded-lg border border-sky-100 bg-sky-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-sky-800">
                    <span class="font-semibold">{{ $this->matkulTanpaBobotCount }}</span> mata kuliah belum memiliki bobot penilaian.
                </p>
                <button
                    type="button"
                    wire:click="openBobotMassalForm"
                    class="inline-flex items-center justify-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-sm font-medium text-sky-700 shadow-border transition hover:bg-sky-100"
                >
                    <i data-lucide="percent" class="h-4 w-4" aria-hidden="true"></i>
                    Tetapkan Bobot Massal
                </button>
            </div>
        @endif
    ...
    {{-- Modal: Konfirmasi Hapus Kurikulum --}}
    @if ($confirmingDeleteKurikulum)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/40 px-4">
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-border-lg">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    <button type="button" wire:click="cancelDeleteKurikulum" class="rounded-lg px-4 py-2 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50 shadow-border">
                        Batal
                    </button>
                    <button type="button" wire:click="deleteKurikulum" class="rounded-lg bg-rose-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-rose-700">
                        Hapus
                    </button>
                </div>
            </div>
            ...
            <div class="flex max-h-full w-full max-w-3xl flex-col rounded-2xl bg-white shadow-border-lg">
                <div class="flex items-center justify-between gap-3 border-b border-neutral-200 px-6 py-4">
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                        <button
                            type="button"
                            wire:click="syncMatkulFromMaster"
                            wire:loading.attr="disabled"
                            wire:target="syncMatkulFromMaster"
                            class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium text-neutral-700 transition hover:bg-neutral-100 shadow-border disabled:opacity-60"
                            title="Samakan kode, nama, SKS dengan tabel matkul"
                        >
                            <i data-lucide="refresh-cw" class="h-4 w-4" aria-hidden="true" wire:loading.class="animate-spin" wire:target="syncMatkulFromMaster"></i>
                            <span class="hidden sm:inline">Sinkronisasi Mata Kuliah</span>
                        </button>
                ...
                <div class="flex-1 overflow-y-auto px-6 py-4">
                    ...
                    <dl class="divide-y divide-neutral-100">
                        ...
                        @if ($km->kurikulum)
                            <div class="grid grid-cols-1 gap-1 py-2.5 sm:grid-cols-[minmax(0,200px)_1fr] sm:gap-4">
                                <dt class="text-xs font-semibold uppercase tracking-wide text-neutral-500">Kurikulum</dt>
                                <dd class="text-sm text-neutral-900">{{ $km->kurikulum->nama }} ({{ $km->kurikulum->kode }})</dd>
                            </div>
                        @endif
                    </dl>
    ...
    {{-- Modal: Tetapkan Bobot Massal --}}
    @if ($showBobotMassalForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/40 px-4 py-8">
            <div class="flex max-h-full w-full max-w-lg flex-col rounded-2xl bg-white shadow-border-lg">
                <div class="flex items-center justify-between border-b border-neutral-200 px-6 py-4">
                    <h3 class="text-base font-semibold text-neutral-900">Tetapkan Bobot Nilai Massal</h3>
                    <button type="button" wire:click="closeBobotMassalForm" class="rounded-lg p-1 text-neutral-400 transition hover:bg-neutral-100 hover:text-neutral-600">
                        <i data-lucide="x" class="h-4 w-4" aria-hidden="true"></i>
                    </button>
                </div>
                <form wire:submit="saveBobotMassalForm" class="flex flex-1 flex-col overflow-hidden">
                    <div class="max-h-[60vh] overflow-y-auto px-6 py-4">
                        <p class="mb-4 text-sm text-neutral-600">
                            Bobot akan diterapkan ke <span class="font-semibold text-neutral-900">{{ $this->matkulTanpaBobotCount }}</span> mata kuliah yang belum memiliki bobot nilai. Mata kuliah yang sudah memiliki bobot tidak akan diubah.
                        </p>
                        @error('bobotMassalForm')
                            <p class="mb-4 rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 ring-1 ring-rose-100">{{ $message }}</p>
                        @enderror
                        ...
                        @else
                                    ...
                                    <li class="flex items-center gap-3">
                                        ...
                                        <div class="flex items-center gap-1">
                                            <input
                                                ...
                                                max="100"
                                                step="0.01"
                                                wire:model="bobotMassalForm.{{ $jenis->id }}"
                                                class="w-24 rounded-lg px-3 py-2 text-right text-sm tabular-nums outline-none focus:border-neutral-900 focus:ring-2 focus:ring-neutral-900/10 shadow-border"
                                            />
                                            <span class="text-sm text-neutral-400">%</span>
                                        </div>
                            ...
                            <div class="mt-4 flex items-center justify-between border-t border-neutral-100 pt-4">
                                <span class="text-sm font-medium text-neutral-600">Total</span>
                                <span class="text-sm font-semibold tabular-nums {{ $this->totalBobotMassalForm() > 100 ? 'text-rose-600' : 'text-neutral-900' }}">
                                    {{ number_format($this->totalBobotMassalForm(), 2) }}%
                                </span>
                            </div>
                        @endif
                    </div>
                    <div class="flex justify-end gap-2 border-t border-neutral-200 px-6 py-4">
                        <button type="button" wire:click="closeBobotMassalForm" class="rounded-lg px-4 py-2 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50 shadow-border">
                            Batal
                        </button>
                        <button
                            type="submit"
                            @disabled($this->jenisPenilaianOptions->isEmpty() || $this->totalBobotMassalForm() > 100 || $this->matkulTanpaBobotCount === 0)
                            class="inline-flex items-center gap-2 rounded-lg bg-neutral-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                            <i data-lucide="percent" class="h-4 w-4" aria-hidden="true"></i>
                            Terapkan
