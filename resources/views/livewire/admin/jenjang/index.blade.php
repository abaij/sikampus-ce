@section('title', 'Jenjang — ' . config('app.name'))
@section('header_title', 'Jenjang')
@section('header_subtitle', 'Data master jenjang pendidikan')
@section('header_icon', 'layers')
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($jenjangList as $jenjang)
                        <tr wire:key="jenjang-{{ $jenjang->id }}">
                            <td class="px-4 py-3 font-medium text-slate-900">{{ $jenjang->nama }}</td>
                            <td class="px-4 py-3 text-slate-600">{{ $jenjang->kode ?? '—' }}</td>
                            <td class="px-4 py-3 text-slate-600">{{ $jenjang->deskripsi ?? '—' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-10 text-center text-slate-500">Belum ada data jenjang.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <h3 class="text-base font-semibold text-slate-900">Hapus jenjang?</h3>
