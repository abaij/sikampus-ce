@section('title', ($jenjangId ? 'Ubah' : 'Tambah') . ' Jenjang — ' . config('app.name'))
@section('header_title', ($jenjangId ? 'Ubah' : 'Tambah') . ' Jenjang')
...
@section('breadcrumb')
    @include('admin.partials.breadcrumb', ['items' => [
        ['label' => 'Pengaturan'],
        ['label' => 'Jenjang', 'route' => route('admin.jenjang.index')],
        ['label' => $jenjangId ? 'Ubah' : 'Tambah'],
    ]])
@endsection
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="rounded-2xl bg-white p-6 shadow-border">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-neutral-700">Nama Jenjang *</label>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('admin.jenjang.index') }}" class="rounded-lg px-4 py-2.5 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50 shadow-border">
                Batal
            </a>
