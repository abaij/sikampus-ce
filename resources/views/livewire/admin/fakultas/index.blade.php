...
@section('page_actions')
    @if (\App\Support\PanelAccess::can(auth()->user(), 'fakultas', 'create'))
        <a
                ...
                <tbody class="divide-y divide-neutral-100">
                    @forelse ($fakultasList as $fakultas)
                        <tr wire:key="fakultas-{{ $fakultas->id }}">
                            <td class="px-4 py-3 font-medium text-neutral-900">{{ $fakultas->nama }}</td>
                            <td class="px-4 py-3 text-neutral-600">{{ $fakultas->kode ?? '—' }}</td>
                            <td class="px-4 py-3 text-neutral-600">{{ $fakultas->dekan?->nama ?? '—' }}</td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $fakultas->status === 'inactive' ? 'bg-neutral-100 text-neutral-600' : 'bg-emerald-50 text-emerald-700' }}">
                                    {{ $fakultas->status === 'inactive' ? 'Nonaktif' : 'Aktif' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1">
                                    @if (\App\Support\PanelAccess::can(auth()->user(), 'fakultas', 'update'))
                                        <a
                                    ...
                                    @endif
                                    @if (\App\Support\PanelAccess::can(auth()->user(), 'fakultas', 'delete'))
                                        <button
                                            type="button"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-10 text-center text-neutral-500">Belum ada data fakultas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/40 px-4">
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-border-lg">
                <h3 class="text-base font-semibold text-neutral-900">Hapus fakultas?</h3>
