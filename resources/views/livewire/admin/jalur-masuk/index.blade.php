                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($jalurMasukList as $jalurMasuk)
                        <tr wire:key="jalur-masuk-{{ $jalurMasuk->id }}">
                            <td class="px-4 py-3 font-medium text-slate-900">{{ $jalurMasuk->nama }}</td>
                            <td class="px-4 py-3 text-slate-600">{{ $jalurMasuk->is_free_of_charge ? 'Ya' : 'Tidak' }}</td>
                            <td class="px-4 py-3 text-slate-600">{{ $jalurMasuk->has_selection ? 'Ya' : 'Tidak' }}</td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $jalurMasuk->status === 'inactive' ? 'bg-slate-100 text-slate-600' : 'bg-emerald-50 text-emerald-700' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-10 text-center text-slate-500">Belum ada data jalur masuk.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                <h3 class="text-base font-semibold text-slate-900">Hapus jalur masuk?</h3>
