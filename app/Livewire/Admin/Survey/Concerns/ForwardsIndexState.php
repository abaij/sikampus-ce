...
namespace App\Livewire\Admin\Survey\Concerns;
 ...
 * polanya dengan App\Livewire\Admin\Jadwal\Concerns\ForwardsIndexState.
 */
trait ForwardsIndexState
{
    ...
    protected function resolveBackUrl(): void
    {
        $forwarded = collect(request()->query())
            ->only(['search', 'is_active', 'page'])
            ->filter(fn ($value) => $value !== null && $value !== '')
            ->all();

        $this->returnQuery = http_build_query($forwarded);
        $this->backUrl = $this->returnQuery === ''
            ? route('admin.administrasi.survey')
            : route('admin.administrasi.survey').'?'.$this->returnQuery;
    }
}
